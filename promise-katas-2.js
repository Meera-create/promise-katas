...
//adding async makes this function async
const food = async () => {
 const food= await fetch('food');
 return food.data;
};
...
const cat = async () => {
    const cat= await fetch('cats');
    const catsarray=cat.data.cats
    return catsarray.sort();
};
...
const dog = async() => {
  const doggie= await fetch('dogs');
    const dogsarray=doggie.data.dogs
    return dogsarray[1];
};
